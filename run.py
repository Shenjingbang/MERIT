...
def train(args, model, train_features, dev_features):
    ...
    new_layer = ["extractor", "bilinear","new_bil","conbine"]
    ...
    for batch in tqdm(dataloader, desc=f"Evaluating batches"):
        ...
        with torch.no_grad():
            ...
            if "attns" in outputs: # attention recorded
                ...
                attns.extend([a.to(torch.float16).cpu().numpy() for a in attn])
                if args.save_attn and len(attns)>=len(features)//10:
                    ...
                    with open(attns_path, "ab") as f:
                        pickle.dump(attns, f)
                    attns.clear()

    if args.save_attn and len(attns)>0:
        attns_path = os.path.join(args.load_path, f"{os.path.splitext(args.test_file)[0]}.attns")   
        print(f"saving attentions into {attns_path} ...")
        with open(attns_path, "ab") as f:
            pickle.dump(attns, f)
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,local_files_only=True
    )
